...
- 验收：`v1.0-acceptance.md` 全清单闭环（A 核心动线 / B 统计纠错 / C 通知开屏 / D 场景时段 / E 适配无障碍由使用者日常走查；F 工程收口由开发侧执行）。
- 可选增强（不阻塞 1.0）：八主题中景素材铺开（素材到位一张接一张，prompt 见 `docs/design/art-generation-prompts.md` §3）。
